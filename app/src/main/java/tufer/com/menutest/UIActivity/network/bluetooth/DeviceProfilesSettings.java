...
import tufer.com.menutest.UIActivity.network.networkfove.SettingsPreferenceFragment;


/**
 * This preference fragment presents the user with all of the profiles for a
 * particular device, and allows them to be individually connected (or
 * disconnected).
 */
public final class DeviceProfilesSettings extends SettingsPreferenceFragment implements
    ...
    private final HashMap<LocalBluetoothProfile, CheckBoxPreference> mAutoConnectPrefs = new HashMap<LocalBluetoothProfile, CheckBoxPreference>();
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        if (preference == mDeviceNamePref) {
            ...
            return false; // checkbox will update from
                          // onDeviceAttributesChanged() callback
        } else {
            return false;
        }
    ...
    private void askDisconnect(Context context, final LocalBluetoothProfile profile) {
        ...
        String message = context.getString(R.string.bluetooth_disable_profile_message, profileName,
                name);
        ...
        mDisconnectDialog = Utils.showDisconnectDialog(context, mDisconnectDialog,
                disconnectListener, title, Html.fromHtml(message));
    }
