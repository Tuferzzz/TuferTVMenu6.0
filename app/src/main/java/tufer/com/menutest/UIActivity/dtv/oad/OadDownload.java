...
package tufer.com.menutest.UIActivity.dtv.oad;
...
public class OadDownload extends MstarBaseActivity {
    ...
    private boolean onTimeoutReceived = false;
    ...
    protected void onStop() {
        Log.d(TAG, "onStop(), onTimeoutReceived = " + onTimeoutReceived);
        if (true == onTimeoutReceived) {
            mTvOadManager.stopOad(false);
        } else {
    ...
    private void updateOadDownloadInfo(int value) {
        if (value <= 100) {
            ...
            TvCommonManager.getInstance().updateSystem(Constant.OAD_UPGRADE_FILE);
            OadDownload.this.finish();
        }
    }
