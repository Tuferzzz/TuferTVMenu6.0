...
import tufer.com.menutest.UIActivity.dtv.lnb.dvb.LnbOptionActivity;
...
public class LNBSettingActivity extends MstarBaseActivity {
    ...
    private Thread mThreadDiSEqCChange = null;

    private Thread mThreadDtvManualScan = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mComboBtnDiSEqC_1_0 = new ComboButton(this, getResources().getStringArray(
            ...
            @Override
            public void doUpdate() {
                mProgressDialog.setTitle(R.string.str_lnboptting_diseqc_change);
                mProgressDialog.show();

                if (mThreadDiSEqCChange == null || mThreadDiSEqCChange.getState() == Thread.State.TERMINATED ) {
                    /* Change DiSEqC may be time consuming, use another thread to avoid ANR */
                    mThreadDiSEqCChange = new Thread(new Runnable() {
                        @Override
                        public void run() {
                            Log.d(TAG, "New Thread Starting Change DiSEqC !");
                            updateCurrentEditSatelliteInfo();
                    ...
                    mThreadDiSEqCChange.start();
                } else {
                    Log.d(TAG, "Abort, another thread is already starting changing DiSEqC.");
                }
            }
        };
    ...
    protected void onResume() {
        ...
        mProgressDialog.setTitle(R.string.str_lnbsetting_frequencies_set);
        mProgressDialog.show();
        if (mThreadDtvManualScan == null || mThreadDtvManualScan.getState() == Thread.State.TERMINATED ) {
            /* setDvbsScanParam and setDtvManualScanByFreq may be time consuming, use another thread to avoid ANR */
            mThreadDtvManualScan = new Thread(new Runnable() {
                @Override
                public void run() {
                    Log.d(TAG, "New Thread Starting Change scanCurrentFreq!");
                    scanCurrentFreq();
                    if (null != mProgressDialog) {
                        mProgressDialog.dismiss();
                    }
            ...
            mThreadDtvManualScan.start();
        } else {
            Log.d(TAG, "Abort, another thread is already starting changing setDvbsScanParam and setDtvManualScanByFreq.");
        }
    };
