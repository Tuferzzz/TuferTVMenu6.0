...
public class AutoTuneOptionActivity extends MstarBaseActivity {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == MSG_ONCREAT) {
                if (mTvSystem == TvCommonManager.TV_SYSTEM_ATSC) {
                    ...
                    Utility.ttsSepakLinearLayout(viewholder_autotune.linear_cha_autotuning_tuningtype);
                    OnFocusChangeListener focusChangeListener = new OnFocusChangeListener() {
                        @Override
                        public void onFocusChange(View v, boolean hasFocus) {
                            if (hasFocus) {
                                LinearLayout ll = (LinearLayout)v;
                                Utility.ttsSepakLinearLayout(ll);
                            }
                        }
                    };
                    viewholder_autotune.linear_cha_autotuning_tuningtype.setOnFocusChangeListener(focusChangeListener);
                    viewholder_autotune.linear_cha_autotuning_antenna_type.setOnFocusChangeListener(focusChangeListener);
                    viewholder_autotune.button_search.setOnFocusChangeListener(new OnFocusChangeListener() {
                        @Override
                        public void onFocusChange(View v, boolean hasFocus) {
                            if (hasFocus) {
                                TvCommonManager.getInstance().speakTtsDelayed(
                                    viewholder_autotune.button_search.getText().toString()
            ...
            case KeyEvent.KEYCODE_DPAD_RIGHT:
                switch (currentid) {
                    ...
                    case R.id.linearlayout_cha_autotuning_antenna_type:
                        ...
                        TvCommonManager.getInstance().speakTtsDelayed(
                            mOptionScanAntennaTypes[mAntennaType]
                            , TvCommonManager.TTS_QUEUE_FLUSH
                            , TvCommonManager.TTS_SPEAK_PRIORITY_NORMAL
                            , TvCommonManager.TTS_DELAY_TIME_100MS);
                        return true;
    ...
    private void registerListeners() {
        ...
        viewholder_autotune.linear_cha_autotuning_tuningtype.setOnClickListener(new OnClickListener() {
            @Override
    ...
    private void countrySelected(int index) {
        ...
        Log.d(TAG, "countrySelected: country = " + mCountryIndex);
        if (TvChannelManager.TV_ROUTE_DVBC == mCurrentRoute
            ...
            if (true == Tools.isEwbsSupport()) {
                ...
                if (false == settings.getBoolean(Constant.PREFERENCES_IS_EWBS_AREA_CODE_SELECTED, false)) {
                    Intent intent = new Intent();
                    intent.putExtra("forAutoTuning", true);
                    intent.setClass(AutoTuneOptionActivity.this, EWBSInfoActivity.class);
                    startActivity(intent);
                    return;
                }
            }
