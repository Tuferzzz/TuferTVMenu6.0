...
import tufer.com.menutest.Util.Constant.SignalProgSyncStatus;
import tufer.com.menutest.Util.Constant;
import tufer.com.menutest.Util.TvEvent;
...
public class SourceInfoActivity extends MstarBaseActivity {
    ...
    private final int CMD_TTS_OUTPUT = 0x04;
    ...
    private TextView mSource_info_tvname_value;
    ...
    private TextView mSource_epg_event_name_value;

    private TextView mSource_epg_event_rating;

    private TextView mSource_info_fta;
    ...
    private String mTtsChStr = "";

    private String mTtsEvStr = "";
        ...
        public boolean onScreenSaverMode(int what, int arg1) {
            if ((TvCommonManager.INPUT_SOURCE_HDMI <= mInputSource)
                ...
                if (SignalProgSyncStatus.STABLE_SUPPORT_MODE == arg1) {
                    mHandler.sendEmptyMessage(CMD_SCREEN_SAVER);
                    return true;
                }
            }
            return false;
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case CMD_SCREEN_SAVER:
                    ...
                    break;
                case CMD_TTS_OUTPUT:
                    String chNum = mSource_info_tvnumber_value.getText().toString();
                    if (chNum.equals("0")) {
                        /* no any channel exist */
                        break;
                    }
                    String chStr = chNum + ", " + Utility.getStrLimited(mSource_info_tvname_value.getText().toString(), Constant.TTS_CHANNEL_NAME_MAX_LENGTH);
                    String evStr = "";

                    if (mInputSource == TvCommonManager.INPUT_SOURCE_DTV) {
                        evStr = Utility.getStrLimited(mSource_epg_event_name_value.getText().toString(), Constant.TTS_EVENT_NAME_MAX_LENGTH);
                    }
                    if (!mTtsChStr.equals(chStr)) {
                        /* If the channel number and name are different, try to make TTS to speak again. */
                        String str = chStr;
                        mTtsChStr = new String(chStr);
                        if (!evStr.isEmpty()) {
                            str = str + ", " + evStr;
                            mTtsEvStr = new String(evStr);
                        }
    ...
    protected void onResume() {
        ...
        if (mInputSource == TvCommonManager.INPUT_SOURCE_HDMI
            ...
            mHandler.sendEmptyMessageDelayed(CMD_SIGNAL_LOCK, 1000);
        } else {
            if (mTvSystem == TvCommonManager.TV_SYSTEM_ATSC) {
                //FIXME: use SN notification to update info
                updateSourceInfo();
                // this will call updateSourceInfo() delay.
                mHandler.sendEmptyMessageDelayed(CMD_SIGNAL_LOCK, 1000);
            } else {
                // this will call updateSourceInfo() delay.
                mHandler.sendEmptyMessageDelayed(CMD_SIGNAL_LOCK, 300);
            }
        }
        ...
        // Notify the the password prompt for parental block is forced revealed
        // if exists.
    ...
    protected void onPause() {
        ...
        if (false == mIsBackRoot) {
            // Notify the the password prompt for parental block is not forced
            // revealed anymore
    ...
    private void setInvisible() {
        if (mInputSource == TvCommonManager.INPUT_SOURCE_ATV
            ...
            mSource_info_tvname_value.setVisibility(View.INVISIBLE);
        }
    }
    ...
    private void checkInputSourceAndInitView() {
        ...
        if (mInputSource == TvCommonManager.INPUT_SOURCE_ATV) {
            ...
            mSource_info_tvname_value = (TextView) findViewById(R.id.source_info_tvname_value);
            ...
            mSource_info_program_name.requestFocus();
            ...
            mSource_epg_event_name_value = (TextView) findViewById(R.id.source_info_epg_name_value);
            mSource_epg_event_rating = (TextView) findViewById(R.id.source_info_epg_rating_dtv);
            mSource_info_fta = (TextView) findViewById(R.id.source_info_epg_fta);
            mSource_info_genre = (TextView) findViewById(R.id.source_info_genre);
            if (TvCommonManager.TV_SYSTEM_ATSC != mTvSystem) {
                mSource_epg_event_name.setVisibility(View.GONE);
                mSource_epg_event_name_value.setVisibility(View.GONE);
                mSource_epg_event_rating.setVisibility(View.GONE);
                mSource_info_fta.setVisibility(View.GONE);
            } else {
                mSource_epg_event_rating.setVisibility(View.VISIBLE);
            }
    ...
    private void clearTvPartialSourceInfo() {
        if (mInputSource == TvCommonManager.INPUT_SOURCE_ATV) {
            ...
            mSource_epg_event_name_value.setText("");
            mSource_epg_event_rating.setText("");
            mSource_info_genre.setText("");
            if (TvCommonManager.TV_SYSTEM_ATSC == mTvSystem) {
                mSource_epg_event_rating.setText("");
                mSource_info_fta.setText("");
            }
        }
    }
    ...
    private void updateChannelInfo() {
        ...
        if (mInputSource == TvCommonManager.INPUT_SOURCE_DTV) {
            ...
            ProgramInfo CurrProg_Info = null;
            if (mTvSystem == TvCommonManager.TV_SYSTEM_ATSC) {
                ...
                mSource_info_tvname.setText(getString(R.string.str_textview_record_chaneel_name)
                        + " : ");
                mSource_info_tvname_value.setText(name);
                ...
                if (pfEvtInfo != null) {
                    mSource_epg_event_name_value.setText(pfEvtInfo.sName);
                }
                ...
                if (false == CurrProg_Info.isScramble) {
                    mSource_info_fta.setText("FTA");
                    mSource_info_fta.setVisibility(View.VISIBLE);
                } else {
                    mSource_info_fta.setVisibility(View.GONE);
                }
            } else {
                    ...
                    if (mTvSystem == TvCommonManager.TV_SYSTEM_ISDB) {
                        mSource_info_tvname
                                .setText(getString(R.string.str_textview_record_chaneel_name)
                                        + " : ");
                        mSource_info_tvname_value
                                .setText(CurrProg_Info.serviceName);
                        ...
                        mSource_info_tvname
                                .setText(getString(R.string.str_textview_record_chaneel_name)
                                        + " : ");
                        mSource_info_tvname_value
                                .setText(getCurProgrameName(CurrProg_Info.serviceType));
                        mSource_info_tvnumber
                                .setText(getString(R.string.str_cha_dtvmanualtuning_channelno)
                                        + " : ");
                ...
                if (null != mDtvEitInfo) {
                    String text = mSource_info_program_name.getText().toString();
                    if (!text.equals(mDtvEitInfo.eitCurrentEventPf.eventName)) {
                ...
                if (mTvSystem != TvCommonManager.TV_SYSTEM_ISDB) {
                    Time nextProgramBaseTime = mTvTimerManager.getCurrentTvTime();
                    nextProgramBaseTime.set(nextProgramBaseTime.toMillis(true));
                    EpgEventInfo epgEventInfo = TvEpgManager.getInstance().getDvbEventInfoByTime(
                               CurrProg_Info.serviceType, CurrProg_Info.number, nextProgramBaseTime);

                    if (null != epgEventInfo) {
                        Log.d(TAG, "updateChannelInfo, Genre: " + getString(R.string.str_epg_event_genre));
                        mSource_info_genre.setText(getString(R.string.str_epg_event_genre)
                              + getGenreString(epgEventInfo.genre));
                    } else {
                        Log.d(TAG, "updateChannelInfo, Genre: Unknown");
                        mSource_info_genre.setText(getString(R.string.str_epg_event_genre));
                    }
                    mSource_info_genre.setVisibility(View.VISIBLE);
                }
                ...
                if (mTvSystem != TvCommonManager.TV_SYSTEM_ATSC) {
                    if (null != mDtvEitInfo) {
                        String startTimeFmtStr = "HH:mm";
                        String endTimeFmtStr = "HH:mm";

                        if (TvCountry.NEWZEALAND == mCountry) {
                            startTimeFmtStr = "EEE, d MMM yyyy HH:mm:ss";
                            if ((mDtvEitInfo.eitCurrentEventPf.stStartTime.year == mDtvEitInfo.eitCurrentEventPf.stEndTime.year)
                                ...
                                endTimeFmtStr = "HH:mm:ss";
                            } else {
                                /* test case: nz_si03_221, nz_si03_043 */
                                endTimeFmtStr = "EEE, d MMM yyyy HH:mm:ss";
                            }
                        }
                        String startTimeDispStr = null;
                        String endTimeDispStr = null;

                        if (TvCountry.NEWZEALAND == mCountry) {
                            final String TIME_FORMAT_NZDT = " NZDT";
                            final String TIME_FORMAT_NZST = " NZST";
                            startTimeDispStr = new SimpleDateFormat(startTimeFmtStr)
                                .format(new Date(mDtvEitInfo.eitCurrentEventPf.stStartTime
                                        .toMillis(false)));
                            endTimeDispStr = new SimpleDateFormat(endTimeFmtStr)
                                .format(new Date(mDtvEitInfo.eitCurrentEventPf.stEndTime
                                        .toMillis(false)));
                            ...
                            calendar.setTimeInMillis(mDtvEitInfo.eitCurrentEventPf.startTime * 1000L);
                            startTimeDispStr = String.format("%02d:%02d", calendar.get(Calendar.HOUR_OF_DAY), calendar.get(Calendar.MINUTE));

                            calendar.setTimeInMillis(mDtvEitInfo.eitCurrentEventPf.endTime * 1000L);
                            endTimeDispStr = String.format("%02d:%02d", calendar.get(Calendar.HOUR_OF_DAY), calendar.get(Calendar.MINUTE));
                        }

                        if (0 == mDtvEitInfo.eitCurrentEventPf.durationInSeconds) {
                ...
                if (null != mDtvEitInfo) {
                    ...
                    String guidance = mTvEpgManager.getEventDescriptor(CurrProg_Info.serviceType,
                            CurrProg_Info.number, mDtvEitInfo.eitCurrentEventPf.stStartTime,
                            TvEpgManager.EPG_GUIDANCE_DESCRIPTION);
            ...
            if (TvCommonManager.TV_SYSTEM_ISDB == mTvSystem) {
                /*
                 * There is no need to display channel name if input source is
                 * ATV.
                 */
                mSource_info_tvname.setText("");
                mSource_info_tvname_value.setText("");
                ...
                mSource_info_tvname.setText(getString(R.string.str_textview_record_chaneel_name)
                        + " : ");
                mSource_info_tvname_value.setText(getCurProgrameName());
            }
    ...
    private void updateAudioInfo(boolean isAD) {
        ...
        if (audioInfo.audioInfos.length > 0) {
            if (audioInfo.audioInfos[audioInfo.currentAudioIndex] != null) {
                mStr = Utility.getAudioTypeString(
                        audioInfo.audioInfos[audioInfo.currentAudioIndex].audioType,
                        audioInfo.audioInfos[audioInfo.currentAudioIndex].aacProfileAndLevel);
    ...
    private void updateIsCcExist() {
        if (mTvSystem == TvCommonManager.TV_SYSTEM_ISDB) {
            ...
            DTVSpecificProgInfo dvtSpecProgInfo = mTvChannelManager.getCurrentProgramSpecificInfo();
            if ((null != dvtSpecProgInfo) && (true == dvtSpecProgInfo.ccService)) {
                mSource_info_cc.setText(getString(R.string.str_dtv_source_info_cc_exist));
            } else {
                mSource_info_cc.setText(getString(R.string.str_dtv_source_info_cc_noexist));
            }
    ...
    private void selectIconByInputSource() {
        ...
        switch (mInputSource) {
            ...
            case TvCommonManager.INPUT_SOURCE_CVBS:
                setSourceInfo(R.drawable.grid_menu_avi, "CVBS");
                break;
            case TvCommonManager.INPUT_SOURCE_CVBS2:
                setSourceInfo(R.drawable.grid_menu_avi, "CVBS2");
                break;
            ...
            case TvCommonManager.INPUT_SOURCE_HDMI:
                if (TvCommonManager.getInstance().isHdmiSignalMode() == true) {
                    setSourceInfo(R.drawable.grid_menu_hdmi, Utility.getHdmiDispStr(mInputSource));
                } else {
    ...
    public void majorMinorToIcon(int majorNum, int minorNum) {
         ...
         * case: MajorNum = 15, MinorNum = 65535(0xFFFF), channel number will be
         * displayed as "15" in channellist menu. And we accept that user inputs
         * "15" to tune to the channel, it needs to convert minor channel number
    ...
    public void updateSourceInfo() {
        ...
        if (mInputSource == TvCommonManager.INPUT_SOURCE_ATV
            ...
            mSource_info_tvname_value.setVisibility(View.VISIBLE);
            if (mTvSystem == TvCommonManager.TV_SYSTEM_ATSC) {
                mHandler.removeMessages(CMD_TTS_OUTPUT);
                mHandler.sendEmptyMessageDelayed(CMD_TTS_OUTPUT, TvCommonManager.TTS_DELAY_TIME_100MS);
            }
        }
        ...
        if (SwitchPageHelper.goToMenuPage(this, keyCode) == true) {
            ...
            if (mInputSource == TvCommonManager.INPUT_SOURCE_DTV) {
                Utility.scrollPosition(mSource_info_description,
                        (keyCode == KeyEvent.KEYCODE_PAGE_UP) ? Utility.SCROLL_DIRECTION_UP
                                : Utility.SCROLL_DIRECTION_DOWN);
                return true;
            }
        }
    ...
    private BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(TvIntent.ACTION_SIGNAL_LOCK)) {
                mHandler.sendEmptyMessageDelayed(CMD_SIGNAL_LOCK, 1000);// this
    ...
    private void forcePasswordPrompt(boolean bForceReveal) {
        ...
        if (true == bForceReveal) {
            // Notify the the password prompt for parental block is forced
            // revealed if exists.
