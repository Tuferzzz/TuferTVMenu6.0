...
package tufer.com.menutest.UIActivity.dtv.ews;
...
public class EWSActivity extends Activity {
    ...
    private final int DISASTER_PROPOSAL = 0xFF;
    ...
    private BroadcastReceiver mLocalBroadcastReveiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            String action = intent.getAction();
            if (action.equals(TvIntent.ACTION_EWS_FINISH_ACTIVITY)) {
                EWSActivity.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        finish();
                    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        //mEwsInfo.level = EwsInfo.LEVEL_AWAS;
        //mEwsInfo.level = EwsInfo.LEVEL_SIAGA;
        //mEwsInfo.level = EwsInfo.LEVEL_WASPADA;
        ...
        filter.addAction(TvIntent.ACTION_EWS_FINISH_ACTIVITY);
    ...
    protected void onDestroy() {
        ...
        if (null != mLocalBroadcastReveiver) {
            LocalBroadcastManager.getInstance(this).unregisterReceiver(mLocalBroadcastReveiver);
            mLocalBroadcastReveiver = null;
        }
        super.onDestroy();
    }
    ...
    private void loadView() {
        ...
        if (null != mEwsInfo) {
            ...
            if (EwsInfo.LEVEL_AWAS == mEwsInfo.level) {
                ...
                float px = 600 * getResources().getDisplayMetrics().density;
                mLocation.setMaxWidth((int)px);

                TextView status = (TextView) findViewById(R.id.status_text);
                mStatusInfo = status.getText().toString();
                ...
                float px = 450 * getResources().getDisplayMetrics().density;
                mLocation.setMaxWidth((int)px);
                ...
                mStatusInfo = getResources().getString(R.string.str_ews_status_waspada);
                ...
                float px = 340 * getResources().getDisplayMetrics().density;
                mLocation.setMaxWidth((int)px);
            }
        }
    }
    ...
    private void updateInfo() {
        if (null != mEwsInfo) {
            ...
            if (DISASTER_PROPOSAL == mEwsInfo.type) {
                mType.setImageResource(R.drawable.ews_proposal);
            }
            mAuthority = (ImageView) findViewById(R.id.disaster_authority);
            ...
            String dspStr = mEwsInfo.code.toString();
            if (!mEwsInfo.information.toString().isEmpty()) {
                if (!dspStr.isEmpty()) {
                    dspStr += ", ";
                }
                dspStr += mEwsInfo.information.toString();
            }
            if (!dspStr.isEmpty()) {
                dspStr += ", Daerah Anda: ";
            }
            dspStr += mStatusInfo;
            mInformation.setText(dspStr);
        }
    }
}
