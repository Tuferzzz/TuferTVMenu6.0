        ...
        if (flag == true)// stop tuning
        {
            switch (mTvChannelManager.getTuningStatus()) {
                ...
                case TvChannelManager.TUNING_STATUS_DTV_SCAN_PAUSING:
                    ...
                    int dvbt2RouteIndex = mTvChannelManager
                            .getSpecificDtvRouteIndex(TvChannelManager.TV_ROUTE_DVBT2);
                    int currentDvbRouteIndex = mTvChannelManager.getCurrentDtvRouteIndex();
                    if ((currentDvbRouteIndex != dvbtRouteIndex)
                            && (currentDvbRouteIndex != dvbt2RouteIndex)
                        ...
                        if (mTvChannelManager.getUserScanType() == mTvChannelManager.TV_SCAN_ALL) {
                            ...
                            if (res == false) {
                                Log.e(TAG, "startAtvAutoTuning Error!!!");
                            }
                        } else {
                            mTvChannelManager.changeToFirstService(
