...
package tufer.com.menutest.UIActivity.tuning.dvb;
...
import tufer.com.menutest.UIActivity.tuning.AutoTuneOptionActivity;
import tufer.com.menutest.UIActivity.tuning.ChannelTuning;
import tufer.com.menutest.UIActivity.tuning.DTVAutoTuneOptionActivity;
...
public class DvbsDTVAutoTuneOptionActivity extends MstarBaseActivity {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_MENU:
                Intent intent = new Intent(TvIntent.ACTION_AUTOTUNING_OPTION);
        ...
        public void onClick(View view) {
            switch (view.getId()) {
                case R.id.linearlayout_cha_dvbsdtvautotuning_tuneoption: {
                    ...
                    intent.putExtra("dvbsScanType", (int)mComboBtnSearch.getIdx());
                    startActivity(intent);
                    finish();
                    break;
                }
    ...
    private OnCancelListener satCancelListener = new OnCancelListener() {
        ...
        public void onCancel(DialogInterface dialog) {
            ...
            // if there is no selected satellite, set to first satellite.
            int satNum = 0;
            if (mMultiSatelliteList.length > 0) {
                satNum = mMultiSatelliteList[mMultiSatelliteList.length - 1];
            }
    ...
    private OnCancelListener tpCancelListener = new OnCancelListener() {
        ...
        public void onCancel(DialogInterface dialog) {
            ...
            // if there is no selected transponder, set to first transponder.
            int tpNum = 0;
            if (mMultiTransponderList.length > 0) {
                tpNum = mMultiTransponderList[mMultiTransponderList.length - 1];
            }
    ...
    private void createMainPageItem() {
        mComboBtnScanMode = new ComboButton(this, mScanMode,
                R.id.linearlayout_cha_dvbsdtvautotuning_scanmode, 1, 2, ComboButton.DIRECT_SWITCH) {
        };
        mComboBtnServiceType = new ComboButton(this, mServiceType,
                R.id.linearlayout_cha_dvbsdtvautotuning_servicetype, 1, 2, ComboButton.DIRECT_SWITCH) {
        };
        mComboBtnNetworkSearch = new ComboButton(this, mNetworkSearchOption,
                R.id.linearlayout_cha_dvbsdtvautotuning_networksearch, 1, 2, ComboButton.DIRECT_SWITCH) {
        };
        mComboBtnSearch = new ComboButton(this, mSearchOption,
                R.id.linearlayout_cha_dvbsdtvautotuning_tuneoption, 1, 2, ComboButton.DIRECT_SWITCH) {
        };
