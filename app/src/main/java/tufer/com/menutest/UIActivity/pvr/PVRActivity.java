...
import tufer.com.menutest.UIActivity.pvr.USBDiskSelecter.usbListener;
            ...
            case MKeyEvent.KEYCODE_CC:
                if ((TvCommonManager.getInstance().getCurrentTvSystem() == TvCommonManager.TV_SYSTEM_ISDB)
                    ...
                    if (TvCommonManager.getInstance().getCurrentTvSystem() == TvCommonManager.TV_SYSTEM_ATSC) {
                        caption = TvCcManager.getInstance().getNextClosedCaptionMode();
                        TvCcManager.getInstance().setClosedCaptionMode(caption);
                        TvCcManager.getInstance().stopCc();
                        if (TvCcManager.CLOSED_CAPTION_OFF != caption) {
                            TvCcManager.getInstance().startCc();
                        }
                        resId = R.array.str_arr_option_closed_caption;
                    } else {
