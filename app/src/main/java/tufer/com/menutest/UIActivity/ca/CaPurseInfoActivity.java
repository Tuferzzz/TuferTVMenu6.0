...
public class CaPurseInfoActivity extends Activity {
    ...
    protected Handler handler = new Handler() {
        ...
        public void handleMessage(android.os.Message msg) {
            ...
            if (msg.what == LittleDownTimer.TIME_OUT_MSG) {
                ...
                Intent intent = new Intent(CaPurseInfoActivity.this, MainActivity.class);
    ...
    private void getInitData() {
        ...
        if (SlotIDs.sSlotIDsState == CaErrorType.RETURN_CODE.ST_CA_RC_OK.getRetCode()) {
            if (SlotIDs.pbySlotID.length() > 0) {
                ...
                for (int i = 0; i < len; i++) {
                    ...
                    CaSlotInfo SlotInfo = tvCaManager.CaGetSlotInfo(Short.parseShort(OperatorId),
                            (short) slotId[i]);
                    if (SlotInfo == null
                            || SlotInfo.sSlotInfoState != CaErrorType.RETURN_CODE.ST_CA_RC_OK.getRetCode()) {
                        continue;
                    }
        ...
        } else if (SlotIDs.sSlotIDsState == CaErrorType.RETURN_CODE.ST_CA_RC_CARD_INVALID.getRetCode()) {
            toastShow(R.string.st_ca_rc_card_invalid);
        } else if (SlotIDs.sSlotIDsState == CaErrorType.RETURN_CODE.ST_CA_RC_POINTER_INVALID.getRetCode()) {
            toastShow(R.string.st_ca_rc_pointer_invalid);
        } else if (SlotIDs.sSlotIDsState == CaErrorType.RETURN_CODE.ST_CA_RC_DATA_NOT_FIND.getRetCode()) {
