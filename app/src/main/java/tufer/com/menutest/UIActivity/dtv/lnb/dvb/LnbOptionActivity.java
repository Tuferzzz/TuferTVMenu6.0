...
package tufer.com.menutest.UIActivity.dtv.lnb.dvb;
                    ...
                    public void onClick(View v) {
                        switch (v.getId()) {
                            case R.id.lnb_option_motor_none:
                                Log.d(TAG, "onClick() - lnb_option_motor_none");
                                mCurrentSatInfo.motorDiSEqCVer = SatelliteInfo.DISEQC_NONE;
                                break;
                            case R.id.lnb_option_motor_diseqc_1_2:
                                Log.d(TAG, "onClick() - lnb_option_motor_diseqc_1_2");
                                mCurrentSatInfo.position = Integer.valueOf(mTvDvbChannelManager.getUnusedMotorPosition()).shortValue();
                                mCurrentSatInfo.motorDiSEqCVer = SatelliteInfo.DISEQC_1_2;
                                break;
                            case R.id.lnb_option_motor_diseqc_1_3:
                                Log.d(TAG, "onClick() - lnb_option_motor_diseqc_1_3");
                                mCurrentSatInfo.position = Integer.valueOf(mTvDvbChannelManager.getUnusedMotorPosition()).shortValue();
                                mCurrentSatInfo.motorDiSEqCVer = SatelliteInfo.DISEQC_1_3;
                                break;
                            default:
                                Log.e(TAG, "onClick() - this is a error id in page motor !!");
                                break;
                        }

                        if ( mTvDvbChannelManager.updateSatelliteInfo(mCurrentSatNum,  mCurrentSatInfo) != true ) {
                            Log.e(TAG, "onClick() - updateSatelliteInfo Fail");
                        }
                        setResultToLNBSettingPage(v);
                        finish();
                    }
