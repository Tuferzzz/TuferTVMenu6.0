...
public class PALATVManualTuning extends MstarBaseActivity {
    ...
    private final int USER_CHANGE_CHANNEL_TIMEOUT = 1000;

    private final int UPDATE_TUNE_ICON_TIMEOUT = 500;

    private final int UPDATE_TUNE_ICON_NOTIFINCATION = 2001;
    ...
    private Handler mUpdateTuneIconHandler = null;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mUpdateTuneIconHandler = new Handler() {
            @Override
            public void handleMessage(Message msg) {
                updateTuneIcon(mLinearLayoutSearchFineTune, DIRECTION_NONE);
            }
        };
            ...
            public boolean onKey(View view, int keyCode, KeyEvent keyEvent) {
                ...
                if (keyEvent.getAction() == KeyEvent.ACTION_DOWN) {
                    if (KeyEvent.KEYCODE_DPAD_RIGHT == keyCode) {
                        ...
                        updateTuneIcon((LinearLayout) view, DIRECTION_RIGHT);
                        mUpdateTuneIconHandler.removeMessages(UPDATE_TUNE_ICON_NOTIFINCATION);
                        mUpdateTuneIconHandler.sendEmptyMessageDelayed(UPDATE_TUNE_ICON_NOTIFINCATION, UPDATE_TUNE_ICON_TIMEOUT);
                        return true;
                    } else if (KeyEvent.KEYCODE_DPAD_LEFT == keyCode) {
                        if (mTvChannelManager.getTuningStatus() == TvChannelManager.TUNING_STATUS_NONE) {
                            mTvChannelManager.startAtvManualTuning(3 * 1000, nCurrentFrequency,
