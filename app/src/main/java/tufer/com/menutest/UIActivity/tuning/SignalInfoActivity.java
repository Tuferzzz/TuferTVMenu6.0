...
public class SignalInfoActivity extends MstarBaseActivity {
    ...
    private LinearLayout mLinearChInfo;

    private TextView mTextviewChVal;

    private LinearLayout mLinearFrequencyInfo;
    ...
    private LinearLayout mLinearSymbolRate;

    private TextView mTextviewSymbolRateVal;
    ...
    private void findViewId() {
        ...
        mLinearChInfo = (LinearLayout) findViewById(
            R.id.linearlayout_cha_channelinfo);
        mTextviewChVal = (TextView) findViewById(
            R.id.textview_cha_channelinfo_ch_val);

        mLinearFrequencyInfo = (LinearLayout) findViewById(
        ...
        mCabQamTable = getResources().getStringArray(
            R.array.str_arr_signalinfo_qam_cab);
        mDvbtQamTable = getResources().getStringArray(
            R.array.str_arr_signalinfo_qam_dvbt);
        ...
        mLinearSymbolRate = (LinearLayout) findViewById(
            R.id.linearlayout_cha_symbolrate);
        mTextviewSymbolRateVal = (TextView) findViewById(
            R.id.textview_cha_symbolrate_val);
    }
    ...
    private void setChannelAndFrequencyInfo() {
        ...
        if (true == Utility.isATSC()) {
            mLinearFrequencyInfo.setVisibility(View.GONE);
        } else {
            RfInfo rfInfo = mTvChannelManager.getRfInfo(TvChannelManager.RF_INFO, mCurrSignalInfo.rfNumber);
            ...
            if (true != Utility.isISDB()) {
                ...
                if ((mTvChannelManager.getCurrentDtvRouteIndex() == dvbsRouteIndex)
                    || (mTvChannelManager.getCurrentDtvRouteIndex() == dvbs2RouteIndex)) {
                    /* There is no channel info in DVBS/DVBS2 system */
                    mLinearChInfo.setVisibility(View.GONE);
                ...
                if ((mTvChannelManager.getCurrentDtvRouteIndex() == dvbtRouteIndex)
                    ...
                    || (mTvChannelManager.getCurrentDtvRouteIndex() == dtmbRouteIndex)) {
                    mTextviewChVal.setText(rfInfo.rfName);
                }
            }
        ...
        try {
            ...
            if (mode == DtvProgramSignalInfo.EnumProgramDemodType.E_PROGRAM_DEMOD_DVB_T
                ...
                if (mCurrSignalInfo.amMode < mDvbtQamTable.length) {
                    str = mDvbtQamTable[mCurrSignalInfo.amMode];
                    mTextviewModulationVal.setText(str);
                }
            } else if (mode == DtvProgramSignalInfo.EnumProgramDemodType.E_PROGRAM_DEMOD_DVB_C) {
                if (mCurrSignalInfo.amMode < mCabQamTable.length) {
                    str = mCabQamTable[mCurrSignalInfo.amMode];
                    mTextviewModulationVal.setText(str);
                }
            } else if (mode == DtvProgramSignalInfo.EnumProgramDemodType.E_PROGRAM_DEMOD_DVB_S) {
                mTextviewModulationVal.setText(R.string.str_cha_tv_route_dvbs);
            } else {
                mLinearModulation.setVisibility(View.GONE);
            }
        } catch (TvOutOfBoundException e) {
            mLinearSymbolRate.setVisibility(View.GONE);
            e.printStackTrace();
        }
    }
    ...
    private void setSymbolRate() {
        EnumProgramDemodType mode;
        try {
            mode = mCurrSignalInfo.getModulationMode();
            if (mode == DtvProgramSignalInfo.EnumProgramDemodType.E_PROGRAM_DEMOD_DVB_C) {
                String str;
                str = "" + mCurrSignalInfo.symbolRate;
                mTextviewSymbolRateVal.setText(str);
            } else {
                mLinearSymbolRate.setVisibility(View.GONE);
            }
        } catch (TvOutOfBoundException e) {
