...
public final class DockEventReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Intent.ACTION_DOCK_EVENT.equals(intent.getAction())
            ...
            /*
             * Reconnect to the dock if: 1) it is a dock 2) it is disconnected
             * 3) the disconnect is initiated remotely 4) the dock is still
             * docked (check can only be done in the Service)
             */
            if (device == null) {
                if (DEBUG)
                    Log.d(TAG, "Device is missing");
            ...
            if (newState == BluetoothProfile.STATE_DISCONNECTED
                ...
                // Too bad, the dock state can't be checked from a
                // BroadcastReceiver.
                Intent i = new Intent(intent);
                i.setClass(context, DockService.class);
                beginStartingService(context, i);
