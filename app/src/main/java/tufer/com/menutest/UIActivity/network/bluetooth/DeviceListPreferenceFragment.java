 ...
 */
public abstract class DeviceListPreferenceFragment extends SettingsPreferenceFragment implements
        BluetoothCallback {
    ...
    final WeakHashMap<CachedBluetoothDevice, BluetoothDevicePreference> mDevicePreferenceMap = new WeakHashMap<CachedBluetoothDevice, BluetoothDevicePreference>();
    ...
    void addCachedDevices() {
        ...
        for (CachedBluetoothDevice cachedDevice : cachedDevices) {
        	Log.d(TAG, "cachedDevice=========>"+cachedDevice);
    ...
    public void onDeviceAdded(CachedBluetoothDevice cachedDevice) {
    	...
    	Log.d(TAG, "=====111==="+mDevicePreferenceMap.get(cachedDevice));
        if (mDevicePreferenceMap.get(cachedDevice) != null) {
            return;
        }

        // Prevent updates while the list shows one of the state messages
        Log.d(TAG, "====222===="+mLocalAdapter.getBluetoothState());
        if (mLocalAdapter.getBluetoothState() != BluetoothAdapter.STATE_ON)
            return;

        
        Log.d(TAG, "====333===="+mFilter.matches(cachedDevice.getDevice()));
    ...
    void createDevicePreference(CachedBluetoothDevice cachedDevice) {
        BluetoothDevicePreference preference = new BluetoothDevicePreference(getActivity(),
                cachedDevice);
