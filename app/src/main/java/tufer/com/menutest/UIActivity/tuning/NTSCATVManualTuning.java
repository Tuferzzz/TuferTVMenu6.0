...
public class NTSCATVManualTuning extends MstarBaseActivity {
    ...
    /**
     * The variable mCurChannelNumber acquired from Supernova library stands for
     * the internal channel number of the current channel If one'd like to get
     * the surface number for show in OSD/MENU/INFO/...,etc, it can be acquired
     * by calling surfaceNumber = Utility.getATVDisplayChNum(mCurChannelNumber);
     */
    private int mCurChannelNumber = 0;

    /**
     * The variable mNewChannelNumber stands for the internal channel number
     * prepared for next tuning, This variabl keep the mCurChannelNumber stays
     * unchanged when channel tuning get failed. Set mCurChannelNumber =
     * mNewChannelNumber after the tuning is successful to update the UI.
     */
    private int mNewChannelNumber = 0;

    /**
     * The variable mBufferInputNumber only used when user input number directly
     * this is a surface number and only for display purpose Get the internal
     * channel number by calling internalNumber =
     * Utility.getATVRealChNum(mBufferInputNumber)
     */
    ...
    private class AtvScanEventListener implements OnAtvScanEventListener {
        ...
        public boolean onAtvScanEvent(int what, int arg1, int arg2, Object info) {
            if ((TvChannelManager.TVPLAYER_ATV_AUTO_TUNING_SCAN_INFO == what)
                || (TvChannelManager.TVPLAYER_ATV_MANUAL_TUNING_SCAN_INFO == what)) {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mMinCh = mTvChannelManager.getProgramCtrl(TvChannelManager.ATV_PROG_CTRL_GET_MIN_CHANNEL,
                0, 0);
        mMaxCh = mTvChannelManager.getProgramCtrl(TvChannelManager.ATV_PROG_CTRL_GET_MAX_CHANNEL,
                0, 0);
        if (TvCommonManager.TV_SYSTEM_ISDB == mTvSystem) {
            /*
             * In ISDB system, ATV channel number saving in db is zero-base.
             * getProgramCtrl() returns the result of channel number is not
             * zero-base, it is already +1 for UI displaying. So, we need to
             * reduce 1 for the return value for internal use.
             */
            mMinCh = mMinCh - 1;
            mMaxCh = mMaxCh - 1;
        }
                ...
                .setOnFocusChangeListener(new OnFocusChangeListener() {
                    ...
                    public void onFocusChange(View v, boolean hasFocus) {
                        if (!hasFocus) {
                            ...
                            Utility.ttsSepakLinearLayout(mViewholder.linear_cha_ntscatvmanualtuning_channelnum);
                        }
                    }
                });

        mViewholder.linear_cha_ntscatvmanualtuning_frequency
                .setOnFocusChangeListener(new OnFocusChangeListener() {
                    @Override
                    public void onFocusChange(View v, boolean hasFocus) {
                        if (hasFocus) {
                            Utility.ttsSepakLinearLayout(mViewholder.linear_cha_ntscatvmanualtuning_frequency);
                        }
                    }
                });
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_9:
                if (currentid == R.id.linearlayout_cha_ntscatvmanualtuning_channelnum) {
                    ...
                    mBufferInputNumber = mBufferInputNumber * 10 + (keyCode - KeyEvent.KEYCODE_0);
                    ...
                    mViewholder.text_cha_ntscatvmanualtuning_channelnum_val.setText(Integer
                            .toString(mBufferInputNumber));
    ...
    protected void onResume() {
        ...
        TvChannelManager.getInstance().registerOnChannelInfoEventListener(mChannelInfoEventListener);
    }
    ...
    protected void onPause() {
        ...
        if (null != mChannelInfoEventListener) {
            TvChannelManager.getInstance().unregisterOnChannelInfoEventListener(mChannelInfoEventListener);
    ...
    /**
     * doManualTune, one can update the channel number displayed on OSD after
    ...
    private boolean doManualTune(int newChannelNumber) {
        if (TvCommonManager.TV_SYSTEM_ISDB == mTvSystem) {
            if (TvIsdbChannelManager.CHANNEL_SET_FAIL != TvIsdbChannelManager.getInstance()
                    .setChannel(newChannelNumber, true)) {
                return true;
            } else {
                return false;
            }
            ...
            int ret = mTvChannelManager.getAtvProgramInfo(
                    TvChannelManager.GET_PROGRAMINFO_DIRECT_TUNED, newChannelNumber);
            if (0 < ret) {
