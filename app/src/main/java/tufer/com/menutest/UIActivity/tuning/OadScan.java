...
import tufer.com.menutest.UIActivity.dtv.oad.OadDownload;
...
public class OadScan extends MstarBaseActivity {
    ...
    private boolean mIsAutoOadScan = false;
    ...
    protected void onPause() {
        Log.d(TAG, "onPause()");
        if (true == mIsAutoOadScan) {
            mTvChannelManager.stopDtvScan();
            mIsAutoOadScan = false;
            OadScan.this.finish();
    ...
    private void updateOadScanInfo(DtvEventScan extra) {
        ...
        if (scanStatus == TvChannelManager.DTV_SCAN_STATUS_END) {
            mIsAutoOadScan = false;
            oadScanFinish();
        } else if (scanStatus == TvChannelManager.DTV_SCAN_STATUS_EXIT_TO_DL) {
            mIsAutoOadScan = false;
            oadExitToDownload();
        }
    }
